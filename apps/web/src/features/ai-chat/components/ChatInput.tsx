  ...
  return (
    ...
    >
      ...
      {attachment.status === "success" && (
        <Badge
          variant="secondary"
          className="ms-0.5 h-3.5 px-1 text-[9px] text-emerald-600"
        >
          ✓
        </Badge>
      )}
...
export function ChatInput() {
      ...
      const interval = setInterval(() => {
        ...
        if (progress >= 100) {
          ...
          setAttachments((prev) =>
            prev.map((a) =>
              a.id === att.id
                ? { ...a, status: "success", uploadProgress: 100 }
                : a,
            ),
          );
        } else {
  ...
  return (
    <div className="shrink-0 border-t border-border/50 bg-background/80 backdrop-blur-sm px-4 sm:px-6 py-3">
      {/* biome-ignore lint/a11y/noStaticElementInteractions: drag-drop zone, not a focusable control */}
      <div
        ref={containerRef}
          ...
          {isDragging && (
            ...
            >
              ...
              <p className="text-sm font-medium text-primary/80">
                فایل‌ها را برای پیوست رها کنید
              </p>
            </motion.div>
          )}
        </AnimatePresence>
