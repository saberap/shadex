...
const initialNotifications: Notification[] = [
  ...
  },
  {
    ...
    description:
      "شما ۸۷٪ از سهمیه ۵۰ گیگابایتی فضای ذخیره‌سازی خود را استفاده کرده‌اید.",
    time: "۱ ساعت پیش",
    read: false,
  },
  {
      ...
      {items.map((n) => {
        ...
        return (
          <button
            ...
            key={n.id}
            className={cn(
              "flex w-full items-start gap-3 px-4 sm:px-6 py-4 text-start transition-colors hover:bg-muted/30 cursor-pointer",
              !n.read && "bg-muted/20",
            )}
            onClick={() => onRead(n.id)}
          >
