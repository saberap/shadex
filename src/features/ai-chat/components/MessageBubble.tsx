...
function renderMarkdown(text: string) {
    ...
    return (
      <span key={k}>
        {parts.map((part, pi) => {
          ...
          if (part.startsWith("`") && part.endsWith("`"))
            return (
              <code key={pi} className="rounded bg-muted px-1 py-0.5 text-[11px] font-mono">
                {part.slice(1, -1)}
              </code>
            );
          if (part.startsWith("*") && part.endsWith("*"))
  ...
  for (const line of lines) {
    if (line.startsWith("# ")) {
      flushList();
      result.push(
        <h2 key={key++} className="mt-3 mb-1.5 text-base font-semibold text-foreground">
          {line.slice(2)}
        </h2>
      );
      ...
      flushList();
      result.push(
        <h3 key={key++} className="mt-2.5 mb-1 text-sm font-semibold text-foreground">
          {line.slice(3)}
        </h3>
      );
    } else if (line.startsWith("### ")) {
      flushList();
      result.push(
        <h4 key={key++} className="mt-2 mb-0.5 text-sm font-medium text-foreground">
          {line.slice(4)}
        </h4>
      );
    } else if (/^[-*] /.test(line)) {
...
function ChartMessage({ chartContent }: Pick<Message, "chartContent">) {
  ...
  const xKey = Object.keys(chartContent.data[0] ?? {}).find(
    (k) => typeof chartContent.data[0]?.[k] === "string"
  ) ?? "name";

  return (
    <div className="rounded-xl border border-border/50 bg-card p-4">
      <p className="mb-3 text-sm font-medium text-foreground">{chartContent.title}</p>
      <ResponsiveContainer width="100%" height={220}>
        {chartContent.type === "bar" ? (
          <BarChart data={chartContent.data} margin={{ top: 4, right: 4, left: -20, bottom: 4 }}>
          ...
          <AreaChart data={chartContent.data} margin={{ top: 4, right: 4, left: -20, bottom: 4 }}>
            <CartesianGrid strokeDasharray="3 3" className="stroke-border/40" />
            <XAxis dataKey={xKey} tick={{ fontSize: 10 }} />
            <YAxis tick={{ fontSize: 10 }} />
            <RechartsTooltip contentStyle={{ fontSize: 12 }} />
            {chartContent.keys.map((k) => (
              <Area key={k.key} type="monotone" dataKey={k.key} stroke={k.color} fill={k.color} fillOpacity={0.15} />
            ))}
          </AreaChart>
        ) : (
          <LineChart data={chartContent.data} margin={{ top: 4, right: 4, left: -20, bottom: 4 }}>
            <CartesianGrid strokeDasharray="3 3" className="stroke-border/40" />
            <XAxis dataKey={xKey} tick={{ fontSize: 10 }} />
            <YAxis tick={{ fontSize: 10 }} />
            <RechartsTooltip contentStyle={{ fontSize: 12 }} />
            {chartContent.keys.map((k) => (
              <Line key={k.key} type="monotone" dataKey={k.key} stroke={k.color} dot={false} />
            ))}
          </LineChart>
        )}
      </ResponsiveContainer>
